...
// The examples on the main page, the storefront patterns on the Swiper page.
const pages = {
  '/scroll-carousel/': ['hero', 'bestsellers', 'phone', 'guides', 'chips', 'dates', 'few', 'fewClamped'],
  '/scroll-carousel/swiper': ['teasers', 'products', 'stage', 'gallery', 'belt'],
};
...
for (const width of [390, 768, 1280]) {
  test(`every example at ${width} px`, async ({ page }) => {
    ...
    for (const [url, names] of Object.entries(pages)) {
      await page.goto(url);
      await ready(page);
      ...
      for (const name of names) {
        ...
        const isolate = await page.addStyleTag({
          content: `.intro, .bench-bar, .toc, .case-head, .readout, .api, .code, .closing, .pattern > h3, .pattern > p, .wf-table-wrap { display: none !important; }
